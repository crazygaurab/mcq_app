...
import requests
import gspread
FILE_PATH = "src/MCQ/mcq_algae.ods"
...
def load_questions_from_excel(filepath=None):
    ...
    for index, row in enumerate(quiz):
        # Map user's column names to internal structure keys
        answer_key = str(row[6]).strip().upper()
        ...
        if answer_key in ['A', 'B', 'C', 'D']:
            ...
            print(f"Warning: Skipping question {row[index]} due to invalid answer key.")
            continue

        q = {
            "question": str(row[1]).strip(),
            "options": {
                "option A": str(row[2]).strip(),
                "option B": str(row[3]).strip(),
                "option C": str(row[4]).strip(),
                "option D": str(row[5]).strip(),
            },
...
def main(page: ft.Page):
    ...
    def _update_score_display():
        ...
        score_display.current.value = f"Question: {current_q_index + 1} / {len(questions)} | Score: {score}"
